...
let print = (variable) => console.log(JSON.stringify(variable, null, 2))
...
let analisar = (line) => {
    ...
    line.split(',').map((tuple) => {
            ...
            let distance = parseInt(array.join``)
            let nextPosition = {
                x: position.x,
                y: position.y
            }
...
let manhattan = (point1, point2) => Math.abs(point1.x - point2.x) + Math.abs(point1.y - point2.y)

let solve = (input) => {
    
    let wires = input.split('\n').map(wire => analisar(wire))
    ...
    wires[0].map((segment1) => {
        ...
        wires[1].map((segment2) => {
            ...
            if((segment1.from.x == segment1.to.x) ^ (segment2.from.x == segment2.to.x)) {
                ...
                let vertical = segment1.from.x == segment1.to.x ? segment1 : segment2
                let horizontal = segment1.from.x == segment1.to.x ? segment2 : segment1

                let minX = Math.min(horizontal.from.x, horizontal.to.x)
                let maxX = Math.max(horizontal.from.x, horizontal.to.x)

                let minY = Math.min(vertical.from.y, vertical.to.y)
                let maxY = Math.max(vertical.from.y, vertical.to.y)

                if(vertical.from.x >= minX 
